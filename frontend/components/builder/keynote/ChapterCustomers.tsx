...
/* 02 · The Customers — the iconic scene. Every customer session is a seat in an
   ...
   missing cc_bin that V17 excluded and V18 now lets through. The number that
   changes is real engine output, never asserted. */
...
export function ChapterCustomers() {
  ...
  const changedCount = useMemo(() => evaluations.filter((e) => e.changed).length, [evaluations]);
  ...
  return (
    <Scene id="kc-customers" n="02" label="The Customers" accent="offer-blue" clip="kc-customers" flip={false} environment={<AudienceGlow />} live={audience}>
      <div>
        <Pill accent="offer-blue">The audience · a real replay</Pill>
        <SceneHeadline className="mt-6">
          {status === "ok" ? (
            <>
              Two tokens changed <span className="text-crimson">{changedCount}</span> customer decisions.
            </>
          ) : (
